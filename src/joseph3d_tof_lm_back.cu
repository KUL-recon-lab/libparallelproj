...
__global__ void joseph3d_tof_lm_back_kernel(const float *xstart,
                                           const float *xend,
                                           float *img,
                           ...
                           int threadsperblock)
{
    ...
    float *d_p = nullptr;
    bool free_p = false;
    handle_cuda_input_array(p, &d_p, p_size, free_p, device_id, cudaMemAdviseSetReadMostly);
    ...
    joseph3d_tof_lm_back_kernel<<<num_blocks, threadsperblock>>>(
