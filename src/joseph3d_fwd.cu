                  ...
                  int threads_per_block)
{
    ...
    handle_cuda_input_array(projection_values, &d_projection_values.ptr, sizeof(float) * num_lors, d_projection_values.owns, device_id, cudaMemAdviseSetAccessedBy);
    ...
    cudaGetLastError();
    joseph3d_fwd_kernel<<<num_blocks, threads_per_block>>>(d_lor_start.ptr, d_lor_end.ptr, d_image.ptr,
    ...
    if (d_projection_values.owns)
        cuda_memcpy_d2h(projection_values, d_projection_values.ptr, sizeof(float) * num_lors);
}
